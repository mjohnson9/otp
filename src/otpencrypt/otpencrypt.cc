...
bool encrypt_and_output(std::istream* key_material, std::string s);
std::ifstream* open_key_file(std::string filePath);

int main(int argc, char* argv[]) {
  gflags::SetUsageMessage(std::string("encrypt a phrase using OTP\n") +
  ...
  if (!encrypt_and_output(key_file, input)) {
    key_file->close();
    return 1;
  }
...
bool encrypt_and_output(std::istream* key_material, std::string s) {
  ...
  for (auto plaintext : s) {
    ...
    std::cout << otp::encrypt(plaintext, key);
  }
